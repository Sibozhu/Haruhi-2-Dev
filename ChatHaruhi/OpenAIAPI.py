...
class OpenAIAPI(BaseLLM):
    def __init__(self, model="gpt-3.5-turbo-0125", verbose=False):
        super(OpenAIAPI, self).__init__()

        self.api_key = os.environ['openai_key']
        if not self.api_key:
            raise ValueError("OPENAI_API_KEY must be set as an environment variable or passed to the constructor.")
